...
async function askMistral(message, history, knowledge) {
  if (!MISTRAL_API_KEY) {
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    const useRecommendationModel = knowledge.intent?.kind === 'recommendation' && knowledge.intent.budget_inr == null && MISTRAL_API_KEY && req.body.catalogOnly !== true;
...
// ============================================================
app.use('/api/ev', rateLimit({ windowMs: 60 * 1000, max: 40, standardHeaders: true, legacyHeaders: false }));
function evQuery(value) {
  if (value === undefined) return '';
  if (typeof value !== 'string' || value.length > 100) throw new Error('Invalid vehicle search');
  return value.trim();
}
app.get('/api/ev/search', async (req, res) => {
  let make, model;
  try { make = evQuery(req.query.make); model = evQuery(req.query.model); }
  catch { return res.status(400).json({ success: false, message: 'Use a short brand/model name.' }); }
  if (!make && !model) return res.status(400).json({ success: false, message: 'Enter a brand or model.' });
  const result = await searchVehicles(make, model);
  return res.json({ success: true, ...result,
    message: result.count ? '' : 'No confirmed match in the available sources. Check the spelling or try a model name; this does not mean the vehicle does not exist.' });
});
app.get('/api/ev/compare', async (req, res) => {
  let make1, model1, make2, model2;
  try {
    [make1, model1, make2, model2] = ['make1', 'model1', 'make2', 'model2'].map(k => evQuery(req.query[k]));
  } catch { return res.status(400).json({ success: false, message: 'Use short brand/model names.' }); }
  if (!make1 || !make2) return res.status(400).json({ success: false, message: 'Both brands are required.' });
  const [one, two] = await Promise.all([searchVehicles(make1, model1), searchVehicles(make2, model2)]);
  if (!one.count || !two.count) return res.status(404).json({ success: false,
    message: 'One vehicle could not be confirmed. Specify both model names and retry.', notices: [one.notice, two.notice] });
  return res.json({ success: true, vehicle1: one.vehicles[0], vehicle2: two.vehicles[0],
    notice: 'Comparing the first matching configurations. Select exact models; range test cycles and model years may differ.' });
});
app.get('/api/ev/brands', (req, res) => res.json({
  success: true,
  indian: catalog.brands.filter(b => b.market.includes('India')).map(b => b.make),
  global: catalog.brands.filter(b => b.market.includes('Global')).map(b => b.make),
  exhaustive: false,
}));
// Python RAG retrieves the same evidence as the website.
app.post('/api/ev/context', async (req, res) => {
